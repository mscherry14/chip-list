...
Реализует компоненты `Chip` и `ChipList` --  компонент, отображающий список чипсов и удовлетворяющие следующим требованиям:
...
*\(Данная реализация будет добавлена отдельной веткой, на данный момент состояние: <span style="color:red">е реализовано</span>)*

Так же планируется подготовить проект к сборке в виде библиотеки компонентов (с `example` для демонстрации работы)

*\(Данная реализация будет добавлена отдельной веткой, на данный момент состояние:  $${\color{orange}в процессе}$$, ветка `library`)*
